...
class PendingOperationsMonitor(
    ...
    fun anyOperationInProgress(): Flow<Boolean> = operationsInProgress(*Operation.values())

    fun anySaveOperationInProgress(): Flow<Boolean> =
        operationsInProgress(Operation.SAVES_SYNC_ONE_SHOT, Operation.SAVES_SYNC_PERIODIC)

    fun anyLibraryOperationInProgress(): Flow<Boolean> =
        operationsInProgress(Operation.LIBRARY_INDEX, Operation.CORE_UPDATE)

    fun isDirectoryScanInProgress(): Flow<Boolean> = operationsInProgress(Operation.LIBRARY_INDEX)
